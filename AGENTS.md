...
- **CORS:** Origin-restricted. Same-origin always allowed; cross-origin only if listed in KV key `cors:origins` (comma-separated). No `Access-Control-Allow-Origin` header emitted for unlisted origins. Non-browser clients (no `Origin` header) are unaffected.
