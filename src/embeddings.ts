 ...
 * All calls routed through AI Gateway via aiRun() for observability.
 */
...
export async function embed(ai: Ai, text: string): Promise<number[]> {
  const result = (await aiRun(ai, EMBEDDING_MODEL, { text: [text] })) as { data: number[][] };
...
export async function embedBatch(ai: Ai, texts: string[]): Promise<number[][]> {
  ...
  const result = (await aiRun(ai, EMBEDDING_MODEL, { text: texts })) as { data: number[][] };
  return result.data;
}
