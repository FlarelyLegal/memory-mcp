...
export async function createNamespace(
  db: D1Database,
  opts: { name: string; description?: string; metadata?: Record<string, unknown> },
): Promise<string> {
  const id = generateId();
  await db
    .prepare(
      `INSERT INTO namespaces (id, name, description, metadata) VALUES (?, ?, ?, ?)`,
    )
    .bind(id, opts.name, opts.description ?? null, toJson(opts.metadata ?? null))
    .run();
  return id;
}
...
export async function listNamespaces(db: D1Database): Promise<NamespaceRow[]> {
