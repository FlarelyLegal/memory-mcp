...
export function registerMemoryTools(server: McpServer, env: Env, email: string) {
  server.tool(
    ...
    "Create, update, or delete a memory (knowledge fragment).",
    {
      ...
      content: z.string().optional().describe("Required for create"),
      ...
      entity_ids: z.array(z.string()).optional().describe("Link to entities"),
  ...
  server.tool(
    ...
    "Retrieve memories. Modes: recall (ranked by importance+recency), search (keyword), entity (linked to an entity).",
    {
      ...
      namespace_id: z.string().optional().describe("Required for recall/search"),
      entity_id: z.string().optional().describe("Required for entity mode"),
      query: z.string().optional().describe("Required for search mode"),
      type: z.enum(["fact", "observation", "preference", "instruction"]).optional(),
